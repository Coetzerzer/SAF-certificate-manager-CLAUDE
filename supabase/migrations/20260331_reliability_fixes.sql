-- Reliability fixes migration

-- 3.4: Improve parse_flexible_numeric to handle thousands separators
...
begin
  ...
  -- Detect thousands separator pattern: "1,234.56" or "1.234,56"
  -- If both comma and dot exist, the last one is the decimal separator
  if v_clean ~ '^\-?[\d]{1,3}(,\d{3})+(\.\d+)?$' then
    -- US format: commas are thousands, dot is decimal (e.g., "1,234.56")
    v_clean := replace(v_clean, ',', '');
  elsif v_clean ~ '^\-?[\d]{1,3}(\.\d{3}){2,}(,\d+)?$' then
    -- EU format: dots are thousands, comma is decimal (e.g., "1.234,56")
    v_clean := replace(replace(v_clean, '.', ''), ',', '.');
  else
    -- Simple comma-as-decimal: "5,599" -> "5.599"
    v_clean := replace(v_clean, ',', '.');
  end if;
...
-- 2.4: Add composite index for FIFO query performance
create index if not exists invoice_rows_import_uplift_idx
  on public.invoice_rows(import_id, uplift_date);

-- 3.5: Add CHECK constraints on volume columns
alter table public.invoice_rows
  drop constraint if exists invoice_rows_saf_vol_m3_nonneg;
alter table public.invoice_rows
  add constraint invoice_rows_saf_vol_m3_nonneg
  check (saf_vol_m3 is null or saf_vol_m3 >= 0);

alter table public.certificate_allocation_units
  drop constraint if exists cert_alloc_units_saf_volume_nonneg;
alter table public.certificate_allocation_units
  add constraint cert_alloc_units_saf_volume_nonneg
  check (saf_volume_m3 is null or saf_volume_m3 >= 0);

-- 2.6: Update allocate_simple_certificate to check import is active
-- 2.3: Validate coverage month format before to_date call
-- These are applied via the updated function below.
...
begin
  ...
  -- 2.3: Validate coverage month format before to_date
